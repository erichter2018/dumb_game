            ...
            try:
                ...
                if has_red_blob:
                    socketio.emit('detection', {
                        ...
                        'x': blob[0],
                        'y': blob[1], 
                        'message': f'Red blob detected at ({blob[0]}, {blob[1]})'
                    })
                else:
                    socketio.emit('detection', {
                        'found': False,
...
def check_red_blob():
    ...
    try:
        ...
        window_x, window_y, window_w, window_h = window_bounds
        print(f"[automation] Scanning window for red blobs: ({window_x}, {window_y}) {window_w}x{window_h}", flush=True)
        
        # Capture the full window
        cap = RoiCapture(window_x, window_y, window_w, window_h)
        with cap:
            frame = cap.grab()
            if frame:
                rgb = frame.to_rgb()
                ...
                for i, blob in enumerate(red_blobs):
                    ...
                    print(f"[automation] Capturing blob {i+1}: region ({window_x + capture_x}, {window_y + capture_y}) size {capture_w}x{capture_h}", flush=True)
                    
                    blob_cap = RoiCapture(window_x + capture_x, window_y + capture_y, capture_w, capture_h)
                    with blob_cap:
                                        ...
                                        if rgb is not None:
                                            ...
                                            if red_blobs_detected:
                                                ...
                                                while blue_clicks < 10 and everything_cleanup_running:
                                                    ...
                                                    if rgb is not None and _detect_blue_blob(rgb):
                                                        blue_clicks += 1
                                                        print(f"[automation] Research: Blue blob click #{blue_clicks}", flush=True)
                                                        move_mouse_to(ar_center_x, ar_center_y)
                                                        click(ar_center_x, ar_center_y, hold_ms=100)
...
def detect_red_blobs(rgb: np.ndarray):
    ...
    # More specific red color ranges for the research indicator
    # Targeting the specific red shade in the research icon
    lower_red1 = np.array([0, 120, 120])    # More saturated red
    upper_red1 = np.array([8, 255, 255])
    lower_red2 = np.array([170, 120, 120])  # Higher hue red range  
    ...
    for contour in contours:
        # Filter by area - research indicators are specific size
        area = cv2.contourArea(contour)
        if area < 80 or area > 800:  # Size range for research indicators
            print(f"[automation] REJECTED by area: {area} (need 80-800)", flush=True)
            continue
        
        # Get bounding rectangle
        x, y, w, h = cv2.boundingRect(contour)
        
        # Must be roughly circular (research indicators are round)
        aspect_ratio = w / h
        if not (0.8 <= aspect_ratio <= 1.25):
            print(f"[automation] REJECTED by aspect ratio: {aspect_ratio:.2f} (need 0.8-1.25)", flush=True)
            continue
        
        # Calculate circularity - research indicators should be very circular
        perimeter = cv2.arcLength(contour, True)
        if perimeter == 0:
            continue
        circularity = 4 * np.pi * area / (perimeter * perimeter)
        
        if circularity < 0.65:  # Must be quite circular
            print(f"[automation] REJECTED by circularity: {circularity:.2f} (need >= 0.65)", flush=True)
            continue
        ...
        white_pixels = np.sum(roi_gray > 200)  # Count bright pixels
        ...
        # Research indicators should have some white content (the arrow)
        if white_ratio < 0.15 or white_ratio > 0.6:  # 15-60% white content
            print(f"[automation] REJECTED by white ratio: {white_ratio:.2f} (need 0.15-0.6)", flush=True)
            continue

        # Enhanced pattern detection to distinguish UP ARROWS (correct) from EXCLAMATION POINTS (wrong)
        try:
            print(f"[automation] Analyzing red blob at ({x}, {y}) {w}x{h}, white_ratio={white_ratio:.2f}", flush=True)
            
            # Get binary mask of white pixels
            white_mask = roi_gray > 200
            
            # If there are white pixels, analyze their pattern
            if np.sum(white_mask) > 0:
                # Find white pixel coordinates
                white_coords = np.where(white_mask)
                
                if len(white_coords[0]) > 3:  # Need at least a few pixels
                    y_coords = white_coords[0]
                    x_coords = white_coords[1]
                    
                    # Calculate the spread in x vs y direction
                    y_spread = np.max(y_coords) - np.min(y_coords)
                    x_spread = np.max(x_coords) - np.min(x_coords)
                    
                    if y_spread > 0 and x_spread > 0:
                        white_aspect_ratio = y_spread / x_spread
                        
                        # ENHANCED EXCLAMATION POINT DETECTION: Connected component analysis
                        # Exclamation points have TWO separate white areas (line + dot)
                        # Up arrows have ONE contiguous white area
                        
                        # Use connected component analysis to count separate white regions
                        from scipy.ndimage import label
                        
                        # Create binary mask of white pixels in this blob
                        white_binary = (roi_gray > 200).astype(np.uint8)
                        
                        # Find connected components (separate white regions)
                        labeled_array, num_components = label(white_binary)
                        
                        if num_components >= 2:
                            print(f"[automation] REJECTED: Exclamation point pattern - {num_components} separate white regions (line + dot)", flush=True)
                            continue
                        
                        print(f"[automation] Connected components analysis: {num_components} white region(s) - continuing", flush=True)
                        
                        # EXCLAMATION POINT DETECTION (reject these)
                        # Exclamation points: very tall, very narrow, with a gap pattern
                        if white_aspect_ratio > 3.0 and x_spread <= 3:
                            print(f"[automation] Potential exclamation point detected: aspect_ratio={white_aspect_ratio:.2f}, x_spread={x_spread}", flush=True)
                            
                            # Check for the characteristic exclamation point gap pattern
                            y_min, y_max = np.min(y_coords), np.max(y_coords)
                            if y_max - y_min > 8:  # Tall enough to have gap
                                # Divide into thirds: top (line), middle (gap), bottom (dot)
                                height = y_max - y_min + 1
                                top_third = height // 3
                                bottom_third = 2 * height // 3
                                
                                # Count white pixels in each third
                                top_pixels = np.sum(white_mask[y_min:y_min + top_third, :])
                                middle_pixels = np.sum(white_mask[y_min + top_third:y_min + bottom_third, :])
                                bottom_pixels = np.sum(white_mask[y_min + bottom_third:y_max + 1, :])
                                
                                # Exclamation pattern: top has pixels, middle is sparse, bottom has pixels
                                if top_pixels > 0 and bottom_pixels > 0 and middle_pixels < (top_pixels * 0.3):
                                    print(f"[automation] REJECTED: Exclamation point pattern detected (top:{top_pixels}, mid:{middle_pixels}, bot:{bottom_pixels})", flush=True)
                                    continue  # Skip this blob - it's an exclamation point
                        
                        # UP ARROW DETECTION (prefer these)
                        # Up arrows: more balanced aspect ratio, wider at bottom, narrower at top
                        elif 1.2 <= white_aspect_ratio <= 2.5 and x_spread >= 4:
                            # Check for triangular/arrow pattern - wider at bottom, narrower at top
                            y_min, y_max = np.min(y_coords), np.max(y_coords)
                            height = y_max - y_min + 1
                            
                            if height >= 6:  # Tall enough to analyze
                                # Check width distribution from top to bottom
                                width_top = 0
                                width_bottom = 0
                                
                                # Top third
                                for row in range(y_min, y_min + height // 3):
                                    if row < roi_gray.shape[0]:
                                        row_pixels = np.sum(white_mask[row, :])
                                        if row_pixels > 0:
                                            row_coords = np.where(white_mask[row, :])[0]
                                            if len(row_coords) > 0:
                                                width_top = max(width_top, np.max(row_coords) - np.min(row_coords) + 1)
                                
                                # Bottom third  
                                for row in range(y_min + 2 * height // 3, y_max + 1):
                                    if row < roi_gray.shape[0]:
                                        row_pixels = np.sum(white_mask[row, :])
                                        if row_pixels > 0:
                                            row_coords = np.where(white_mask[row, :])[0]
                                            if len(row_coords) > 0:
                                                width_bottom = max(width_bottom, np.max(row_coords) - np.min(row_coords) + 1)
                                
                                # EXCLAMATION POINT DETECTION: Very narrow top (≤4 pixels) indicates exclamation, not arrow
                                if width_top <= 4 and width_bottom > width_top + 4:
                                    width_ratio = width_top / width_bottom if width_bottom > 0 else 0
                                    # Exclamation points have extreme width ratios (very narrow top vs wider bottom)
                                    if width_ratio < 0.5:
                                        print(f"[automation] REJECTED: Exclamation point pattern - narrow top ({width_top}) vs wide bottom ({width_bottom}), ratio={width_ratio:.2f}", flush=True)
                                        continue
                                
                                # UP ARROW: should have reasonable width at top, expanding to bottom
                                # But not the extreme narrow-to-wide ratio of exclamation points
                                if width_bottom > width_top and width_top >= 3:
                                    width_ratio = width_top / width_bottom if width_bottom > 0 else 0
                                    
                                    # Additional exclamation check: extremely narrow width ratio
                                    if width_ratio < 0.35:
                                        print(f"[automation] REJECTED: Exclamation point pattern - extreme width ratio ({width_ratio:.2f})", flush=True)
                                        continue
                                        
                                    print(f"[automation] UP ARROW pattern detected: top_width={width_top}, bottom_width={width_bottom}, ratio={width_ratio:.2f}", flush=True)
                                    # This looks like a good up arrow - continue processing
                                else:
                                    # Even ambiguous patterns should be checked for exclamation points
                                    # Small blobs are likely exclamation points regardless of width measurements
                                    if w <= 24 and h <= 24 and white_aspect_ratio >= 1.3:
                                        print(f"[automation] REJECTED: Small ambiguous blob (likely exclamation) - size {w}x{h}, aspect={white_aspect_ratio:.2f}", flush=True)
                                        continue
                                    
                                    # Traditional width-based check for larger blobs
                                    if width_top <= 4 and width_bottom <= 15 and white_aspect_ratio > 1.5:
                                        # This could be an exclamation point with inconsistent width measurement
                                        print(f"[automation] REJECTED: Potential exclamation point in ambiguous pattern - top={width_top}, bottom={width_bottom}, aspect={white_aspect_ratio:.2f}", flush=True)
                                        continue
                                    
                                    print(f"[automation] Ambiguous pattern: top_width={width_top}, bottom_width={width_bottom}, aspect={white_aspect_ratio:.2f}", flush=True)
                                    # Continue processing - might still be valid
                        
                        else:
                            # Even if pattern doesn't match UP ARROW criteria, still check for exclamation points
                            # Exclamation points often have low aspect ratios and narrow spreads
                            # ALSO: Add size-based detection for small blobs that wiggle/grow
                            if (white_aspect_ratio <= 2.0 and x_spread <= 20) or (w <= 24 and h <= 24 and white_aspect_ratio >= 1.0):
                                # Do a simple width analysis for exclamation point detection
                                y_min, y_max = np.min(y_coords), np.max(y_coords)
                                height = y_max - y_min + 1
                                ...
                                if height >= 6:  # Tall enough to analyze
                                    ...
                                    # Check for exclamation point pattern (with wiggle tolerance)
                                    # Small blobs (≤24px) with tall aspect ratios are likely exclamation points
                                    if w <= 24 and h <= 24 and white_aspect_ratio >= 1.5:
                                        print(f"[automation] REJECTED: Small blob exclamation point - size {w}x{h}, aspect={white_aspect_ratio:.2f}", flush=True)
                                        continue
                                    
                                    # Traditional width-based detection (for larger/clearer cases)
                                    if width_top <= 4 and width_bottom > width_top + 3:
                                        width_ratio = width_top / width_bottom if width_bottom > 0 else 0
                                        if width_ratio < 0.5:
                                            print(f"[automation] REJECTED: Exclamation point pattern in fallback - narrow top ({width_top}) vs wide bottom ({width_bottom}), ratio={width_ratio:.2f}", flush=True)
                                            continue
                            
                            print(f"[automation] Pattern analysis: aspect_ratio={white_aspect_ratio:.2f}, x_spread={x_spread} (continuing)", flush=True)
                            
                            # CONNECTED COMPONENT ANALYSIS: Count separate white regions
                            # Exclamation points have TWO separate white areas (line + dot)
                            # Up arrows have ONE contiguous white area
                            try:
                                from scipy.ndimage import label
                                ...
                                print(f"[automation] Connected component analysis failed: {comp_error} (continuing)", flush=True)
                                pass
                            
        except Exception as e:
            # If any error in pattern detection, continue with normal processing
            print(f"[automation] Pattern detection error: {e} (continuing)", flush=True)
            pass
        
        # Additional check: look for the specific red color intensity
        roi_hsv = cv2.cvtColor(roi, cv2.COLOR_RGB2HSV)
        red_pixels = np.sum((roi_hsv[:,:,1] > 100) & (roi_hsv[:,:,2] > 100))  # Saturated and bright
        red_ratio = red_pixels / total_pixels
        
        if red_ratio < 0.3:  # Should have significant red content
            print(f"[automation] REJECTED by red ratio: {red_ratio:.2f} (need >= 0.3)", flush=True)
            continue
    ...
    # Sort by circularity and size (best matches first)
    red_blobs.sort(key=lambda b: (b['circularity'], b['area']), reverse=True)
